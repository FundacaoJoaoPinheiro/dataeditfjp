...
var_numeric <- data |> 
  ...
  colnames() |> 
  unlist()
qualitative_var <- c("HS_DISPOSLIXVALOR")
var_numeric <- var_numeric[!(var_numeric %in% c(qualitative_var))]
```
...
df_rules_valid_hs_all <- data |> 
  ...
  dplyr::mutate(
    # Missing
    VI_NA_HS_COBAGUA             = dplyr::if_else(!is.na(HS_COBAGUA), F, NA),
    VI_NA_HS_PRESTAGUA           = dplyr::if_else(!is.na(HS_PRESTAGUA), F, NA),
    VI_NA_HS_COBESGOTO           = dplyr::if_else(!is.na(HS_COBESGOTO), F, NA),
    VI_NA_HS_PRESTESGOTO         = dplyr::if_else(!is.na(HS_PRESTESGOTO), F, NA),
    VI_NA_HS_ESGOTOAD            = dplyr::if_else(!is.na(HS_ESGOTOAD), F, NA),
    VI_NA_HS_DISPOSLIXO          = dplyr::if_else(!is.na(HS_DISPOSLIXO), F, NA),
    VI_NA_HS_LIXO                = dplyr::if_else(!is.na(HS_LIXO), F, NA),
    VI_NA_HS_COLSELETIVA         = dplyr::if_else(!is.na(HS_COLSELETIVA), F, NA),
    VI_NA_HS_LIXOAD              = dplyr::if_else(!is.na(HS_LIXOAD), F, NA),
    VI_NA_HS_COBLIXO             = dplyr::if_else(!is.na(HS_COBLIXO), F, NA),
    VI_NA_HS_POLITICAEXISTENCIA  = dplyr::if_else(!is.na(HS_POLITICAEXISTENCIA), F, NA),
    VI_NA_HS_POLITICAAGUA        = dplyr::if_else(!is.na(HS_POLITICAAGUA), F, NA),
    VI_NA_HS_POLITICAESGOTO      = dplyr::if_else(!is.na(HS_POLITICAESGOTO), F, NA),
    VI_NA_HS_POLITICARESIDUOSSOLIDOS = dplyr::if_else(!is.na(HS_POLITICARESIDUOSSOLIDOS), F, NA),
    VI_NA_HS_POLITICADRENAGEM    = dplyr::if_else(!is.na(HS_POLITICADRENAGEM), F, NA),
    VI_NA_HS_POLITICA            = dplyr::if_else(!is.na(HS_POLITICA), F, NA),
    VI_NA_HS_PLANOSAHSEXISTENCIA = dplyr::if_else(!is.na(HS_PLANOSAHSEXISTENCIA), F, NA),
    VI_NA_HS_PLANOSAHSAGUA       = dplyr::if_else(!is.na(HS_PLANOSAHSAGUA), F, NA),
    VI_NA_HS_PLANOSAHSESGOTO     = dplyr::if_else(!is.na(HS_PLANOSAHSESGOTO), F, NA),
    VI_NA_HS_PLANOSAHSRESIDUOSSOLIDOS = dplyr::if_else(!is.na(HS_PLANOSAHSRESIDUOSSOLIDOS), F, NA),
    VI_NA_HS_PLANOSAHSDRENAGEM   = dplyr::if_else(!is.na(HS_PLANOSAHSDRENAGEM), F, NA),
    VI_NA_HS_PLANOSAN            = dplyr::if_else(!is.na(HS_PLANOSAN), F, NA),
    VI_NA_HS_PARTICIPACAO        = dplyr::if_else(!is.na(HS_PARTICIPACAO), F, NA),
    VI_NA_HS_CONSELHO            = dplyr::if_else(!is.na(HS_CONSELHO), F, NA),
    VI_NA_HS_GESTAO              = dplyr::if_else(!is.na(HS_GESTAO), F, NA),
    VI_NA_HS_REGULACAOAGUA       = dplyr::if_else(!is.na(HS_REGULACAOAGUA), F, NA),
    VI_NA_HS_REGULACAOESGOTO     = dplyr::if_else(!is.na(HS_REGULACAOESGOTO), F, NA),
    VI_NA_HS_REGULACAORESIDUOS   = dplyr::if_else(!is.na(HS_REGULACAORESIDUOS), F, NA),
    VI_NA_HS_REGULACAODRENAGEM   = dplyr::if_else(!is.na(HS_REGULACAODRENAGEM), F, NA),
    VI_NA_HS_DEFICIT             = dplyr::if_else(!is.na(HS_DEFICIT), F, NA),
    VI_NA_HS_DEFICITREL          = dplyr::if_else(!is.na(HS_DEFICITREL), F, NA),
    VI_NA_HS_DPRECARIO           = dplyr::if_else(!is.na(HS_DPRECARIO), F, NA),
    VI_NA_HS_DCOAB               = dplyr::if_else(!is.na(HS_DCOAB), F, NA),
    VI_NA_HS_DONUS               = dplyr::if_else(!is.na(HS_DONUS), F, NA),
    VI_NA_HS_DADENS              = dplyr::if_else(!is.na(HS_DADENS), F, NA),
    VI_NA_HS_DINADEQUADO         = dplyr::if_else(!is.na(HS_DINADEQUADO), F, NA),
    VI_NA_HS_DSEMINFRA           = dplyr::if_else(!is.na(HS_DSEMINFRA), F, NA),
    VI_NA_HS_SEMBANH             = dplyr::if_else(!is.na(HS_SEMBANH), F, NA),
    VI_NA_HS_DDURBADENS          = dplyr::if_else(!is.na(HS_DDURBADENS), F, NA),
    VI_NA_HS_TRATESGOTO          = dplyr::if_else(!is.na(HS_TRATESGOTO), F, NA),
    # Others
...
df_rules_dist_hs_cobagua <- data |> 
  dplyr::filter( # Filtro condicional para variáveis que estão em "ano_base_alter"
    dplyr::case_when(
      "HS_COBAGUA" %in% names(ano_base_alter) ~ ANO >= ano_base_alter["HS_COBAGUA"],
...
df_rules_dist_hs_cobesgoto <- data |> 
  dplyr::filter( # Filtro condicional para variáveis que estão em "ano_base_alter"
    dplyr::case_when(
      "HS_COBESGOTO" %in% names(ano_base_alter) ~ ANO >= ano_base_alter["HS_COBESGOTO"],
      TRUE ~ ANO >= min(ANO, na.rm = T)
    )
  ) |>
...
df_rules_dist_hs_esgotoad <- data |> 
  dplyr::filter( # Filtro condicional para variáveis que estão em "ano_base_alter"
    dplyr::case_when(
      "HS_ESGOTOAD" %in% names(ano_base_alter) ~ ANO >= ano_base_alter["HS_ESGOTOAD"],
      TRUE ~ ANO >= min(ANO, na.rm = T)
    )
  ) |>
...
df_rules_dist_hs_lixo <- data |> 
  dplyr::filter( # Filtro condicional para variáveis que estão em "ano_base_alter"
    dplyr::case_when(
      "HS_LIXO" %in% names(ano_base_alter) ~ ANO >= ano_base_alter["HS_LIXO"],
      TRUE ~ ANO >= min(ANO, na.rm = T)
    )
  ) |>
...
df_rules_dist_hs_lixoad <- data |> 
  dplyr::filter( # Filtro condicional para variáveis que estão em "ano_base_alter"
    dplyr::case_when(
      "HS_LIXOAD" %in% names(ano_base_alter) ~ ANO >= ano_base_alter["HS_LIXOAD"],
      TRUE ~ ANO >= min(ANO, na.rm = T)
    )
  ) |>
...
df_rules_dist_hs_coblixo <- data |> 
  dplyr::filter( # Filtro condicional para variáveis que estão em "ano_base_alter"
    dplyr::case_when(
      "HS_COBLIXO" %in% names(ano_base_alter) ~ ANO >= ano_base_alter["HS_COBLIXO"],
      TRUE ~ ANO >= min(ANO, na.rm = T)
    )
  ) |>
...
df_rules_dist_hs_politica <- data |> 
  dplyr::filter( # Filtro condicional para variáveis que estão em "ano_base_alter"
    dplyr::case_when(
      "HS_POLITICA" %in% names(ano_base_alter) ~ ANO >= ano_base_alter["HS_POLITICA"],
      TRUE ~ ANO >= min(ANO, na.rm = T)
    )
  ) |>
...
df_rules_dist_hs_planosan <- data |> 
  dplyr::filter( # Filtro condicional para variáveis que estão em "ano_base_alter"
    dplyr::case_when(
      "HS_PLANOSAN" %in% names(ano_base_alter) ~ ANO >= ano_base_alter["HS_PLANOSAN"],
      TRUE ~ ANO >= min(ANO, na.rm = T)
    )
  ) |>
...
df_rules_dist_hs_conselho <- data |> 
  dplyr::filter( # Filtro condicional para variáveis que estão em "ano_base_alter"
    dplyr::case_when(
      "HS_CONSELHO" %in% names(ano_base_alter) ~ ANO >= ano_base_alter["HS_CONSELHO"],
      TRUE ~ ANO >= min(ANO, na.rm = T)
    )
  ) |>
...
df_rules_dist_hs_gestao <- data |> 
  dplyr::filter( # Filtro condicional para variáveis que estão em "ano_base_alter"
    dplyr::case_when(
      "HS_GESTAO" %in% names(ano_base_alter) ~ ANO >= ano_base_alter["HS_GESTAO"],
      TRUE ~ ANO >= min(ANO, na.rm = T)
    )
  ) |>
...
df_rules_dist_hs_deficit <- data |> 
  dplyr::filter( # Filtro condicional para variáveis que estão em "ano_base_alter"
    dplyr::case_when(
      "HS_DEFICIT" %in% names(ano_base_alter) ~ ANO >= ano_base_alter["HS_DEFICIT"],
      TRUE ~ ANO >= min(ANO, na.rm = T)
    )
  ) |>
...
df_rules_dist_hs_deficitrel <- data |> 
  dplyr::filter( # Filtro condicional para variáveis que estão em "ano_base_alter"
    dplyr::case_when(
      "HS_DEFICITREL" %in% names(ano_base_alter) ~ ANO >= ano_base_alter["HS_DEFICITREL"],
      TRUE ~ ANO >= min(ANO, na.rm = T)
    )
  ) |>
...
df_rules_dist_hs_dprecario <- data |> 
  dplyr::filter( # Filtro condicional para variáveis que estão em "ano_base_alter"
    dplyr::case_when(
      "HS_DPRECARIO" %in% names(ano_base_alter) ~ ANO >= ano_base_alter["HS_DPRECARIO"],
      TRUE ~ ANO >= min(ANO, na.rm = T)
    )
  ) |>
...
df_rules_dist_hs_dcoab <- data |> 
  dplyr::filter( # Filtro condicional para variáveis que estão em "ano_base_alter"
    dplyr::case_when(
      "HS_DCOAB" %in% names(ano_base_alter) ~ ANO >= ano_base_alter["HS_DCOAB"],
      TRUE ~ ANO >= min(ANO, na.rm = T)
    )
  ) |>
...
df_rules_dist_hs_donus <- data |> 
  dplyr::filter( # Filtro condicional para variáveis que estão em "ano_base_alter"
    dplyr::case_when(
      "HS_DONUS" %in% names(ano_base_alter) ~ ANO >= ano_base_alter["HS_DONUS"],
      TRUE ~ ANO >= min(ANO, na.rm = T)
    )
  ) |>
...
df_rules_dist_hs_dadens <- data |> 
  dplyr::filter( # Filtro condicional para variáveis que estão em "ano_base_alter"
    dplyr::case_when(
      "HS_DADENS" %in% names(ano_base_alter) ~ ANO >= ano_base_alter["HS_DADENS"],
      TRUE ~ ANO >= min(ANO, na.rm = T)
    )
  ) |>
...
df_rules_dist_hs_dinadequado <- data |> 
  dplyr::filter( # Filtro condicional para variáveis que estão em "ano_base_alter"
    dplyr::case_when(
      "HS_DINADEQUADO" %in% names(ano_base_alter) ~ ANO >= ano_base_alter["HS_DINADEQUADO"],
      TRUE ~ ANO >= min(ANO, na.rm = T)
    )
  ) |>
...
df_rules_dist_hs_dseminfra <- data |> 
  dplyr::filter( # Filtro condicional para variáveis que estão em "ano_base_alter"
    dplyr::case_when(
      "HS_DSEMINFRA" %in% names(ano_base_alter) ~ ANO >= ano_base_alter["HS_DSEMINFRA"],
      TRUE ~ ANO >= min(ANO, na.rm = T)
    )
  ) |>
...
df_rules_dist_hs_sembanh <- data |> 
  dplyr::filter( # Filtro condicional para variáveis que estão em "ano_base_alter"
    dplyr::case_when(
      "HS_SEMBANH" %in% names(ano_base_alter) ~ ANO >= ano_base_alter["HS_SEMBANH"],
      TRUE ~ ANO >= min(ANO, na.rm = T)
    )
  ) |>
...
df_rules_dist_hs_ddurbadens <- data |> 
  dplyr::filter( # Filtro condicional para variáveis que estão em "ano_base_alter"
    dplyr::case_when(
      "HS_DDURBADENS" %in% names(ano_base_alter) ~ ANO >= ano_base_alter["HS_DDURBADENS"],
      TRUE ~ ANO >= min(ANO, na.rm = T)
    )
  ) |>
...
df_rules_dist_hs_tratesgoto <- data |> 
  dplyr::filter( # Filtro condicional para variáveis que estão em "ano_base_alter"
    dplyr::case_when(
      "HS_TRATESGOTO" %in% names(ano_base_alter) ~ ANO >= ano_base_alter["HS_TRATESGOTO"],
      TRUE ~ ANO >= min(ANO, na.rm = T)
    )
  ) |>
