...
df_rules_dist_el_quant <- data |> 
  ...
  dplyr::mutate(
    ...
    DF_EL_QUANT_out_munic     = dplyr::if_else(!outlier_detection(EL_QUANT), F, T), # Longitudinal (município)
...
df_rules_dist_el_proge <- data |> 
  ...
  dplyr::mutate(
    ...
    DF_EL_PROGE_out_munic     = dplyr::if_else(!outlier_detection(EL_PROGE), F, T), # Longitudinal (município)
...
df_rules_dist_el_espesc <- data |> 
  ...
  dplyr::mutate(
    ...
    DF_EL_ESPESC_out_munic     = dplyr::if_else(!outlier_detection(EL_ESPESC), F, T), # Longitudinal (município)
...
df_rules_dist_el_ilrhe <- data |> 
  ...
  dplyr::mutate(
    ...
    DF_EL_ILRHE_out_munic     = dplyr::if_else(!outlier_detection(EL_ILRHE), F, T), # Longitudinal (município)
