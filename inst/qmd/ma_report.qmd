...
barplot_data <- dplyr::select(validate::violating(data, check_validate_na), ANO) |> 
  dplyr::group_by(ANO) |>
  dplyr::summarise(
    n = dplyr::n()
  )

## Gráfico barra obs. suspeitas por ano
if(dplyr::n_distinct(barplot_data$ANO) >= 1)
  chart_bar(barplot_data$ANO, barplot_data$n)
...
barplot_data <- dplyr::select(validate::violating(data, check_validate_others), ANO) |> 
  dplyr::group_by(ANO) |>
  dplyr::summarise(
    n = dplyr::n()
  )
...
barplot_data <- dplyr::select(validate::violating(data_aux_ma_icmsecolog, rules_consist_ma_icmsecolog), ANO) |> 
  dplyr::group_by(ANO) |>
  dplyr::summarise(
    n = dplyr::n()
  )
...
barplot_data <- dplyr::select(validate::violating(data_aux_ma_crh, rules_consist_ma_crh), ANO) |> 
  dplyr::group_by(ANO) |>
  dplyr::summarise(
    n = dplyr::n()
  )
...
barplot_data <- dplyr::select(validate::violating(data_aux_ma_cfem, rules_consist_ma_cfem), ANO) |> 
  dplyr::group_by(ANO) |>
  dplyr::summarise(
    n = dplyr::n()
  )
...
barplot_data <- dplyr::select(validate::violating(data, rules_consist_ma_focos), ANO) |> 
  dplyr::group_by(ANO) |>
  dplyr::summarise(
    n = dplyr::n()
  )
...
rules_dist_ma_icmsecolog <- validate::validator(
  ...
  DF_MA_ICMSECOLOG_out_munic     = validate::do_by(MA_ICMSECOLOG, by = IBGE7, fun = outlier_function) == F, # Longitudinal (município)

#  DF_MA_ICMSECOLOG_min_munic     = MA_ICMSECOLOG >= (validate::min_by(dplyr::lag(MA_ICMSECOLOG), by = IBGE7, na.rm = T) / k2), # Longitudinal (município)
  
#  DF_MA_ICMSECOLOG_max_munic     = MA_ICMSECOLOG <= (k2 * validate::max_by(dplyr::lag(MA_ICMSECOLOG), by = IBGE7, na.rm = T)), # Longitudinal (município)
...
barplot_data <- dplyr::select(validate::violating(data, rules_dist_ma_icmsecolog), ANO) |> 
  dplyr::group_by(ANO) |>
  dplyr::summarise(
    n = dplyr::n()
  )
...
rules_dist_ma_crh <- validate::validator(
  ...
  DF_MA_CRH_out_munic     = validate::do_by(MA_CRH, by = IBGE7, fun = outlier_function) == F, # Longitudinal (município)
  
#  DF_MA_CRH_min_munic     = MA_CRH >= (validate::min_by(dplyr::lag(MA_CRH), by = IBGE7, na.rm = T) / k2), # Longitudinal (município)

#  DF_MA_CRH_max_munic     = MA_CRH <= (k2 * validate::max_by(dplyr::lag(MA_CRH), by = IBGE7, na.rm = T)), # Longitudinal (município)
...
barplot_data <- dplyr::select(validate::violating(data, rules_dist_ma_crh), ANO) |> 
  dplyr::group_by(ANO) |>
  dplyr::summarise(
    n = dplyr::n()
  )
...
rules_dist_ma_cfem <- validate::validator(
  ...
  DF_MA_CFEM_out_munic     = validate::do_by(MA_CFEM, by = IBGE7, fun = outlier_function) == F, # Longitudinal (município)
  
  #DF_MA_CFEM_min_munic     = MA_CFEM >= (validate::min_by(dplyr::lag(MA_CFEM), by = IBGE7, na.rm = T) / k2), # Longitudinal (município)
  
  #DF_MA_CFEM_max_munic     = MA_CFEM <= (k2 * validate::max_by(dplyr::lag(MA_CFEM), by = IBGE7, na.rm = T)), # Longitudinal (município)
...
barplot_data <- dplyr::select(validate::violating(data, rules_dist_ma_cfem), ANO) |> 
  dplyr::group_by(ANO) |>
  dplyr::summarise(
    n = dplyr::n()
  )
...
rules_dist_ma_focos <- validate::validator(
  ...
  DF_MA_FOCOS_out_munic     = validate::do_by(MA_FOCOS, by = IBGE7, fun = outlier_function) == F, # Longitudinal (município)
  
  DF_MA_FOCOS_min_munic     = MA_FOCOS >= (validate::min_by(dplyr::lag(MA_FOCOS), by = IBGE7, na.rm = T) / k2), # Longitudinal (município)
  
  DF_MA_FOCOS_max_munic     = MA_FOCOS <= (k2 * validate::max_by(dplyr::lag(MA_FOCOS), by = IBGE7, na.rm = T)), # Longitudinal (município)
  
  DF_MA_FOCOS_med_mov_inf = MA_FOCOS <= (k2 - validate::do_by(MA_FOCOS, by = IBGE7, fun = zoo::rollmedian, k = 3, fill = NA, align = "right")), # Longitudinal (município)
  
   DF_MA_FOCOS_med_mov_sup = MA_FOCOS <= (k2 + validate::do_by(MA_FOCOS, by = IBGE7, fun = zoo::rollmedian, k = 3, fill = NA, align = "right")), # Longitudinal (município)
...
barplot_data <- dplyr::select(validate::violating(data, rules_dist_ma_focos), ANO) |> 
  dplyr::group_by(ANO) |>
  dplyr::summarise(
    n = dplyr::n()
  )
...
rules_dist_ma_cana <- validate::validator(
  ...
  DF_MA_CANA_out_munic     = validate::do_by(MA_CANA, by = IBGE7, fun = outlier_function) == F, # Longitudinal (município)
  
  DF_MA_CANA_min_munic     = MA_CANA >= (validate::min_by(dplyr::lag(MA_CANA), by = IBGE7, na.rm = T) / k2), # Longitudinal (município)
  
  DF_MA_CANA_max_munic     = MA_CANA <= (k2 * validate::max_by(dplyr::lag(MA_CANA), by = IBGE7, na.rm = T)), # Longitudinal (município)
  
  DF_MA_CANA_med_mov_inf = MA_CANA <= (k2 - validate::do_by(MA_CANA, by = IBGE7, fun = zoo::rollmedian, k = 3, fill = NA, align = "right")), # Longitudinal (município)
  
   DF_MA_CANA_med_mov_sup = MA_CANA <= (k2 + validate::do_by(MA_CANA, by = IBGE7, fun = zoo::rollmedian, k = 3, fill = NA, align = "right")), # Longitudinal (município)
...
barplot_data <- dplyr::select(validate::violating(data, rules_dist_ma_cana), ANO) |> 
  dplyr::group_by(ANO) |>
  dplyr::summarise(
    n = dplyr::n()
  )
...
rules_dist_ma_frota <- validate::validator(
  ...
  DF_MA_FROTA_out_munic     = validate::do_by(MA_FROTA, by = IBGE7, fun = outlier_function) == F, # Longitudinal (município)
  
  DF_MA_FROTA_min_munic     = MA_FROTA >= (validate::min_by(dplyr::lag(MA_FROTA), by = IBGE7, na.rm = T) / k1), # Longitudinal (município)
  
  DF_MA_FROTA_max_munic     = MA_FROTA <= (k1 * validate::max_by(dplyr::lag(MA_FROTA), by = IBGE7, na.rm = T)), # Longitudinal (município)
  
  DF_MA_FROTA_med_mov_inf = MA_FROTA <= (k1 - validate::do_by(MA_FROTA, by = IBGE7, fun = zoo::rollmedian, k = 3, fill = NA, align = "right")), # Longitudinal (município)
  
    DF_MA_FROTA_med_mov_sup = MA_FROTA <= (k1 + validate::do_by(MA_FROTA, by = IBGE7, fun = zoo::rollmedian, k = 3, fill = NA, align = "right")), # Longitudinal (município)
...
barplot_data <- dplyr::select(validate::violating(data, rules_dist_ma_frota), ANO) |> 
  dplyr::group_by(ANO) |>
  dplyr::summarise(
    n = dplyr::n()
  )
...
rules_dist_ma_bovino <- validate::validator(
  ...
  DF_MA_BOVINO_out_munic     = validate::do_by(MA_BOVINO, by = IBGE7, fun = outlier_function) == F, # Longitudinal (município)
  
  DF_MA_BOVINO_min_munic     = MA_BOVINO >= (validate::min_by(dplyr::lag(MA_BOVINO), by = IBGE7, na.rm = T) / k1), # Longitudinal (município)
  
  DF_MA_BOVINO_max_munic     = MA_BOVINO <= (k1 * validate::max_by(dplyr::lag(MA_BOVINO), by = IBGE7, na.rm = T)), # Longitudinal (município)
  
  DF_MA_BOVINO_med_mov_inf = MA_BOVINO <= (k1 - validate::do_by(MA_BOVINO, by = IBGE7, fun = zoo::rollmedian, k = 3, fill = NA, align = "right")), # Longitudinal (município)
  
    DF_MA_BOVINO_med_mov_sup = MA_BOVINO <= (k1 + validate::do_by(MA_BOVINO, by = IBGE7, fun = zoo::rollmedian, k = 3, fill = NA, align = "right")), # Longitudinal (município)
...
barplot_data <- dplyr::select(validate::violating(data, rules_dist_ma_bovino), ANO) |> 
  dplyr::group_by(ANO) |>
  dplyr::summarise(
    n = dplyr::n()
  )
...
rules_dist_ma_reflo <- validate::validator(
  ...
  DF_MA_REFLO_out_munic     = validate::do_by(MA_REFLO, by = IBGE7, fun = outlier_function) == F, # Longitudinal (município)
  
  DF_MA_REFLO_min_munic     = MA_REFLO >= (validate::min_by(dplyr::lag(MA_REFLO), by = IBGE7, na.rm = T) / k2), # Longitudinal (município)
  
  DF_MA_REFLO_max_munic     = MA_REFLO <= (k2 * validate::max_by(dplyr::lag(MA_REFLO), by = IBGE7, na.rm = T)), # Longitudinal (município)
  
  DF_MA_REFLO_med_mov_inf = MA_REFLO <= (k2 - validate::do_by(MA_REFLO, by = IBGE7, fun = zoo::rollmedian, k = 3, fill = NA, align = "right")), # Longitudinal (município)
  
    DF_MA_REFLO_med_mov_sup = MA_REFLO <= (k2 + validate::do_by(MA_REFLO, by = IBGE7, fun = zoo::rollmedian, k = 3, fill = NA, align = "right")), # Longitudinal (município)
...
barplot_data <- dplyr::select(validate::violating(data, rules_dist_ma_reflo), ANO) |> 
  dplyr::group_by(ANO) |>
  dplyr::summarise(
    n = dplyr::n()
  )
...
rules_dist_ma_flonativa <- validate::validator(
  ...
  DF_MA_FLONATIVA_out_munic     = validate::do_by(MA_FLONATIVA, by = IBGE7, fun = outlier_function) == F, # Longitudinal (município)
  
  DF_MA_FLONATIVA_min_munic     = MA_FLONATIVA >= (validate::min_by(dplyr::lag(MA_FLONATIVA), by = IBGE7, na.rm = T) / k2), # Longitudinal (município)
  
  DF_MA_FLONATIVA_max_munic     = MA_FLONATIVA <= (k2 * validate::max_by(dplyr::lag(MA_FLONATIVA), by = IBGE7, na.rm = T)), # Longitudinal (município)
   
  DF_MA_FLONATIVA_med_mov_inf = MA_FLONATIVA <= (k2 - validate::do_by(MA_FLONATIVA, by = IBGE7, fun = zoo::rollmedian, k = 3, fill = NA, align = "right")), # Longitudinal (município)
  
    DF_MA_FLONATIVA_med_mov_sup = MA_FLONATIVA <= (k2 + validate::do_by(MA_FLONATIVA, by = IBGE7, fun = zoo::rollmedian, k = 3, fill = NA, align = "right")), # Longitudinal (município)
...
barplot_data <- dplyr::select(validate::violating(data, rules_dist_ma_flonativa), ANO) |> 
  dplyr::group_by(ANO) |>
  dplyr::summarise(
    n = dplyr::n()
  )
...
rules_dist_ma_infra <- validate::validator(
  ...
  DF_MA_INFRA_out_munic     = validate::do_by(MA_INFRA, by = IBGE7, fun = outlier_function) == F, # Longitudinal (município)
  
  DF_MA_INFRA_min_munic     = MA_INFRA >= (validate::min_by(dplyr::lag(MA_INFRA), by = IBGE7, na.rm = T) / k2), # Longitudinal (município)
  
  DF_MA_INFRA_max_munic     = MA_INFRA <= (k2 * validate::max_by(dplyr::lag(MA_INFRA), by = IBGE7, na.rm = T)), # Longitudinal (município)
  
  DF_MA_INFRA_med_mov_inf = MA_INFRA <= (k2 - validate::do_by(MA_INFRA, by = IBGE7, fun = zoo::rollmedian, k = 3, fill = NA, align = "right")), # Longitudinal (município)
  
    DF_MA_INFRA_med_mov_sup= MA_INFRA <= (k2 + validate::do_by(MA_INFRA, by = IBGE7, fun = zoo::rollmedian, k = 3, fill = NA, align = "right")), # Longitudinal (município)
...
barplot_data <- dplyr::select(validate::violating(data, rules_dist_ma_infra), ANO) |> 
  dplyr::group_by(ANO) |>
  dplyr::summarise(
    n = dplyr::n()
  )
...
rules_dist_ma_agro <- validate::validator(
  ...
  DF_MA_AGRO_out_munic     = validate::do_by(MA_AGRO, by = IBGE7, fun = outlier_function) == F, # Longitudinal (município)
  
  DF_MA_AGRO_min_munic     = MA_AGRO >= (validate::min_by(dplyr::lag(MA_AGRO), by = IBGE7, na.rm = T) / k2), # Longitudinal (município)
  
  DF_MA_AGRO_max_munic     = MA_AGRO <= (k2 * validate::max_by(dplyr::lag(MA_AGRO), by = IBGE7, na.rm = T)), # Longitudinal (município)
  
  DF_MA_AGRO_med_mov_inf   = MA_AGRO <= (k2 - validate::do_by(MA_AGRO, by = IBGE7, fun = zoo::rollmedian, k = 3, fill = NA, align = "right")), # Longitudinal (município)
  
    DF_MA_AGRO_med_mov_sup = MA_AGRO <= (k2 + validate::do_by(MA_AGRO, by = IBGE7, fun = zoo::rollmedian, k = 3, fill = NA, align = "right")), # Longitudinal (município)
...
barplot_data <- dplyr::select(validate::violating(data, rules_dist_ma_agro), ANO) |> 
  dplyr::group_by(ANO) |>
  dplyr::summarise(
    n = dplyr::n()
  )
