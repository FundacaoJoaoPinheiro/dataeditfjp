...
chart_label <- c("Suspeito", "Validado", "Missing", "Total")
...
rules_dist_ma_icmsecolog <- validate::validator(
  ...
  DF_MA_ICMSECOLOG_min_munic     = MA_ICMSECOLOG >= (validate::min_by(dplyr::lag(MA_ICMSECOLOG), by = IBGE7, na.rm = T) / k), # Longitudinal (município)
  
  DF_MA_ICMSECOLOG_max_munic     = MA_ICMSECOLOG <= (k * validate::max_by(dplyr::lag(MA_ICMSECOLOG), by = IBGE7, na.rm = T)), # Longitudinal (município)
...
rules_dist_ma_crh <- validate::validator(
  ...
  DF_MA_CRH_min_munic     = MA_CRH >= (validate::min_by(dplyr::lag(MA_CRH), by = IBGE7, na.rm = T) / k), # Longitudinal (município)

  DF_MA_CRH_max_munic     = MA_CRH <= (k * validate::max_by(dplyr::lag(MA_CRH), by = IBGE7, na.rm = T)), # Longitudinal (município)
...
rules_dist_ma_cfem <- validate::validator(
  ...
  DF_MA_CFEM_min_munic     = MA_CFEM >= (validate::min_by(dplyr::lag(MA_CFEM), by = IBGE7, na.rm = T) / k), # Longitudinal (município)
  
  DF_MA_CFEM_max_munic     = MA_CFEM <= (k * validate::max_by(dplyr::lag(MA_CFEM), by = IBGE7, na.rm = T)), # Longitudinal (município)
...
rules_dist_ma_focos <- validate::validator(
  ...
  DF_MA_FOCOS_min_munic     = MA_FOCOS >= (validate::min_by(dplyr::lag(MA_FOCOS), by = IBGE7, na.rm = T) / k), # Longitudinal (município)
  
  DF_MA_FOCOS_max_munic     = MA_FOCOS <= (k * validate::max_by(dplyr::lag(MA_FOCOS), by = IBGE7, na.rm = T)), # Longitudinal (município)
...
rules_dist_ma_cana <- validate::validator(
  ...
  DF_MA_CANA_min_munic     = MA_CANA >= (validate::min_by(dplyr::lag(MA_CANA), by = IBGE7, na.rm = T) / k), # Longitudinal (município)
  
  DF_MA_CANA_max_munic     = MA_CANA <= (k * validate::max_by(dplyr::lag(MA_CANA), by = IBGE7, na.rm = T)), # Longitudinal (município)
...
rules_dist_ma_frota <- validate::validator(
  ...
  DF_MA_FROTA_min_munic     = MA_FROTA >= (validate::min_by(dplyr::lag(MA_FROTA), by = IBGE7, na.rm = T) / k), # Longitudinal (município)
  
  DF_MA_FROTA_max_munic     = MA_FROTA <= (k * validate::max_by(dplyr::lag(MA_FROTA), by = IBGE7, na.rm = T)), # Longitudinal (município)
  ...
  DF_MA_FROTA_compare       = validate::do_by(MA_FROTA, by = IBGE7, fun = compare_dplyr::lag) == T # Longitudinal (município) [comparação]
)
```
...
rules_dist_ma_bovino <- validate::validator(
  ...
  DF_MA_BOVINO_min_munic     = MA_BOVINO >= (validate::min_by(dplyr::lag(MA_BOVINO), by = IBGE7, na.rm = T) / k), # Longitudinal (município)
  
  DF_MA_BOVINO_max_munic     = MA_BOVINO <= (k * validate::max_by(dplyr::lag(MA_BOVINO), by = IBGE7, na.rm = T)), # Longitudinal (município)
...
rules_dist_ma_reflo <- validate::validator(
  ...
  DF_MA_REFLO_min_munic     = MA_REFLO >= (validate::min_by(dplyr::lag(MA_REFLO), by = IBGE7, na.rm = T) / k), # Longitudinal (município)
  
  DF_MA_REFLO_max_munic     = MA_REFLO <= (k * validate::max_by(dplyr::lag(MA_REFLO), by = IBGE7, na.rm = T)), # Longitudinal (município)
...
rules_dist_ma_flonativa <- validate::validator(
  ...
  DF_MA_FLONATIVA_min_munic     = MA_FLONATIVA >= (validate::min_by(dplyr::lag(MA_FLONATIVA), by = IBGE7, na.rm = T) / k), # Longitudinal (município)
  
  DF_MA_FLONATIVA_max_munic     = MA_FLONATIVA <= (k * validate::max_by(dplyr::lag(MA_FLONATIVA), by = IBGE7, na.rm = T)), # Longitudinal (município)
...
rules_dist_ma_infra <- validate::validator(
  ...
  DF_MA_INFRA_min_munic     = MA_INFRA >= (validate::min_by(dplyr::lag(MA_INFRA), by = IBGE7, na.rm = T) / k), # Longitudinal (município)
  
  DF_MA_INFRA_max_munic     = MA_INFRA <= (k * validate::max_by(dplyr::lag(MA_INFRA), by = IBGE7, na.rm = T)), # Longitudinal (município)
...
rules_dist_ma_agro <- validate::validator(
  ...
  DF_MA_AGRO_min_munic     = MA_AGRO >= (validate::min_by(dplyr::lag(MA_AGRO), by = IBGE7, na.rm = T) / k), # Longitudinal (município)
  
  DF_MA_AGRO_max_munic     = MA_AGRO <= (k * validate::max_by(dplyr::lag(MA_AGRO), by = IBGE7, na.rm = T)), # Longitudinal (município)
