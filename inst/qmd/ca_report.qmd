...
format: 
  html:
    ...
    theme:
      light: [flatly, css/ca_custom.scss]
editor: visual
execute:
  echo: false
  warning: false
...
df_rules_valid_ca_all <- data |> 
  ...
  dplyr::mutate(
    # Missing
    VI_NA_CA_MUSEU       = dplyr::if_else(!is.na(CA_MUSEU), F, NA),
    VI_NA_CA_TEATRO      = dplyr::if_else(!is.na(CA_TEATRO), F, NA),
    VI_NA_CA_CINEMA      = dplyr::if_else(!is.na(CA_CINEMA), F, NA),
    VI_NA_CA_BIBLIOTECA  = dplyr::if_else(!is.na(CA_BIBLIOTECA), F, NA),
    VI_NA_CA_CENTROC     = dplyr::if_else(!is.na(CA_CENTROC), F, NA),
    VI_NA_CA_EQUIP       = dplyr::if_else(!is.na(CA_EQUIP), F, NA),
    VI_NA_CA_BANDA       = dplyr::if_else(!is.na(CA_BANDA), F, NA),
    VI_NA_CA_GARTISTICO  = dplyr::if_else(!is.na(CA_GARTISTICO), F, NA),
    VI_NA_CA_ATIVCULT    = dplyr::if_else(!is.na(CA_ATIVCULT), F, NA),
    VI_NA_CA_MEIOC       = dplyr::if_else(!is.na(CA_MEIOC), F, NA),
    VI_NA_CA_ORGAOC      = dplyr::if_else(!is.na(CA_ORGAOC), F, NA),
    VI_NA_CA_INSTCONS    = dplyr::if_else(!is.na(CA_INSTCONS), F, NA),
    VI_NA_CA_TOMBEF      = dplyr::if_else(!is.na(CA_TOMBEF), F, NA),
    VI_NA_CA_TOMBMUN     = dplyr::if_else(!is.na(CA_TOMBMUN), F, NA),
    VI_NA_CA_PCL         = dplyr::if_else(!is.na(CA_PCL), F, NA),
    VI_NA_CA_APRESPC     = dplyr::if_else(!is.na(CA_APRESPC), F, NA),
    VI_NA_CA_GPRESPC     = dplyr::if_else(!is.na(CA_GPRESPC), F, NA),
    VI_NA_CA_AREABIB     = dplyr::if_else(!is.na(CA_AREABIB), F, NA),
    VI_NA_CA_ACERVOBIB   = dplyr::if_else(!is.na(CA_ACERVOBIB), F, NA),
    VI_NA_CA_WEBBIB      = dplyr::if_else(!is.na(CA_WEBBIB), F, NA),
    VI_NA_CA_COMPBIB     = dplyr::if_else(!is.na(CA_COMPBIB), F, NA),
    VI_NA_CA_LEITMESBIB  = dplyr::if_else(!is.na(CA_LEITMESBIB), F, NA),
    VI_NA_CA_EMPMESBIB   = dplyr::if_else(!is.na(CA_EMPMESBIB), F, NA),
    VI_NA_CA_ICMSPATCULT = dplyr::if_else(!is.na(CA_ICMSPATCULT), F, NA),
    VI_NA_CA_FUNDO       = dplyr::if_else(!is.na(CA_FUNDO), F, NA),
    VI_NA_CA_REGISTRO    = dplyr::if_else(!is.na(CA_REGISTRO), F, NA),
    VI_NA_CA_ESTRUOGC    = dplyr::if_else(!is.na(CA_ESTRUOGC), F, NA),
    VI_NA_CA_LEGPAT      = dplyr::if_else(!is.na(CA_LEGPAT), F, NA),
    VI_NA_CA_ARQPUB      = dplyr::if_else(!is.na(CA_ARQPUB), F, NA),
    VI_NA_CA_NUMBIB      = dplyr::if_else(!is.na(CA_NUMBIB), F, NA),
    # Others
