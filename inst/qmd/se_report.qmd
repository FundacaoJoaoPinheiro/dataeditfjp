...
rules_type <- validate::validator(
  ...
  TI_SE_TXMOAVC45A59      = is.numeric(SE_TXMOAVC45A59),
  TI_SE_TXMOATRA15A29     = is.numeric(SE_TXMOATRA15A29),
  TI_SE_TXMOHOMI          = is.numeric(SE_TXMOHOMI),
  TI_SE_TXMOHOMI15A29     = is.numeric(SE_TXMOHOMI15A29),
...
df_rules_valid_se_all <- data |> 
  ...
  dplyr::mutate(
    # Missing
    VI_NA_SE_TXBRUTAMORT       = dplyr::if_else(!is.na(SE_TXBRUTAMORT), F, NA),
    VI_NA_SE_TXBRUTAMORTPAD    = dplyr::if_else(!is.na(SE_TXBRUTAMORTPAD), F, NA),
    VI_NA_SE_TXMOISQCOR45A59   = dplyr::if_else(!is.na(SE_TXMOISQCOR45A59), F, NA),
    VI_NA_SE_TXMOAVC45A59      = dplyr::if_else(!is.na(SE_TXMOAVC45A59), F, NA),
    VI_NA_SE_TXMOATRA15A29     = dplyr::if_else(!is.na(SE_TXMOATRA15A29), F, NA),
    VI_NA_SE_TXMOHOMI          = dplyr::if_else(!is.na(SE_TXMOHOMI), F, NA),
    VI_NA_SE_TXMOHOMI15A29     = dplyr::if_else(!is.na(SE_TXMOHOMI15A29), F, NA),
    VI_NA_SE_TXMOCANCOLUT      = dplyr::if_else(!is.na(SE_TXMOCANCOLUT), F, NA),
    VI_NA_SE_TXMOCANMA         = dplyr::if_else(!is.na(SE_TXMOCANMA), F, NA),
    VI_NA_SE_TXMOCANPUL        = dplyr::if_else(!is.na(SE_TXMOCANPUL), F, NA),
    VI_NA_SE_OBITO60           = dplyr::if_else(!is.na(SE_OBITO60), F, NA),
    VI_NA_SE_NASCBAIXOPESO     = dplyr::if_else(!is.na(SE_NASCBAIXOPESO), F, NA),
    VI_NA_SE_OBINFSIFILS       = dplyr::if_else(!is.na(SE_OBINFSIFILS), F, NA),
    VI_NA_SE_OBTETANONEO       = dplyr::if_else(!is.na(SE_OBTETANONEO), F, NA),
    VI_NA_SE_OBRAIVA           = dplyr::if_else(!is.na(SE_OBRAIVA), F, NA),
    ...
    VI_NA_SE_TETRA             = dplyr::if_else(!is.na(SE_TETRA), F, NA),
    ...
    VI_NA_SE_OBITSEMASSIS      = dplyr::if_else(!is.na(SE_OBITSEMASSIS), F, NA),
    VI_NA_SE_INTERDCV40        = dplyr::if_else(!is.na(SE_INTERDCV40), F, NA),
    VI_NA_SE_INTERDIAB         = dplyr::if_else(!is.na(SE_INTERDIAB), F, NA),
    VI_NA_SE_INTERSAP          = dplyr::if_else(!is.na(SE_INTERSAP), F, NA),
    VI_NA_SE_INTERSAP1A5       = dplyr::if_else(!is.na(SE_INTERSAP1A5), F, NA),
    VI_NA_SE_INTERSAP60        = dplyr::if_else(!is.na(SE_INTERSAP60), F, NA),
    VI_NA_SE_FRATFEMUR60       = dplyr::if_else(!is.na(SE_FRATFEMUR60), F, NA),
    VI_NA_SE_CANCPROST         = dplyr::if_else(!is.na(SE_CANCPROST), F, NA),
    VI_NA_SE_INTMEDCOMPLDESMIC = dplyr::if_else(!is.na(SE_INTMEDCOMPLDESMIC), F, NA),
    VI_NA_SE_PARTODESLOCMIC    = dplyr::if_else(!is.na(SE_PARTODESLOCMIC), F, NA),
...
df_rules_dist_se_txbrutamort <- data |> 
  dplyr::filter( # Filtro condicional para variáveis que estão em "ano_base_alter"
    dplyr::case_when(
      "SE_TXBRUTAMORT" %in% names(ano_base_alter) ~ ANO >= ano_base_alter["SE_TXBRUTAMORT"],
      TRUE ~ ANO >= min(ANO, na.rm = T)
    )
  ) |> 
...
df_rules_dist_se_txbrutamortpad <- data |> 
  dplyr::filter( # Filtro condicional para variáveis que estão em "ano_base_alter"
    dplyr::case_when(
      "SE_TXBRUTAMORTPAD" %in% names(ano_base_alter) ~ ANO >= ano_base_alter["SE_TXBRUTAMORTPAD"],
      TRUE ~ ANO >= min(ANO, na.rm = T)
    )
  ) |> 
...
df_rules_dist_se_txmoisqcor45a59 <- data |> 
  dplyr::filter( # Filtro condicional para variáveis que estão em "ano_base_alter"
    dplyr::case_when(
      "SE_TXMOISQCOR45A59" %in% names(ano_base_alter) ~ ANO >= ano_base_alter["SE_TXMOISQCOR45A59"],
      TRUE ~ ANO >= min(ANO, na.rm = T)
    )
  ) |> 
...
df_rules_dist_se_txmoavc45a59 <- data |> 
  dplyr::filter( # Filtro condicional para variáveis que estão em "ano_base_alter"
    dplyr::case_when(
      "SE_TXMOAVC45A59" %in% names(ano_base_alter) ~ ANO >= ano_base_alter["SE_TXMOAVC45A59"],
      TRUE ~ ANO >= min(ANO, na.rm = T)
    )
  ) |> 
...
df_rules_dist_se_txmoatra15a29 <- data |> 
  dplyr::filter( # Filtro condicional para variáveis que estão em "ano_base_alter"
    dplyr::case_when(
      "SE_TXMOATRA15A29" %in% names(ano_base_alter) ~ ANO >= ano_base_alter["SE_TXMOATRA15A29"],
      TRUE ~ ANO >= min(ANO, na.rm = T)
    )
  ) |> 
...
df_rules_dist_se_txmohomi <- data |> 
  dplyr::filter( # Filtro condicional para variáveis que estão em "ano_base_alter"
    dplyr::case_when(
      "SE_TXMOHOMI" %in% names(ano_base_alter) ~ ANO >= ano_base_alter["SE_TXMOHOMI"],
      TRUE ~ ANO >= min(ANO, na.rm = T)
    )
  ) |> 
...
df_rules_dist_se_txmohomi15a29 <- data |> 
  dplyr::filter( # Filtro condicional para variáveis que estão em "ano_base_alter"
    dplyr::case_when(
      "SE_TXMOHOMI15A29" %in% names(ano_base_alter) ~ ANO >= ano_base_alter["SE_TXMOHOMI15A29"],
      TRUE ~ ANO >= min(ANO, na.rm = T)
    )
  ) |> 
...
df_rules_dist_se_txmocancolut <- data |> 
  dplyr::filter( # Filtro condicional para variáveis que estão em "ano_base_alter"
    dplyr::case_when(
      "SE_TXMOCANCOLUT" %in% names(ano_base_alter) ~ ANO >= ano_base_alter["SE_TXMOCANCOLUT"],
      TRUE ~ ANO >= min(ANO, na.rm = T)
    )
  ) |> 
...
df_rules_dist_se_txmocanma <- data |> 
  dplyr::filter( # Filtro condicional para variáveis que estão em "ano_base_alter"
    dplyr::case_when(
      "SE_TXMOCANMA" %in% names(ano_base_alter) ~ ANO >= ano_base_alter["SE_TXMOCANMA"],
      TRUE ~ ANO >= min(ANO, na.rm = T)
    )
  ) |> 
...
df_rules_dist_se_txmocanpul <- data |> 
  dplyr::filter( # Filtro condicional para variáveis que estão em "ano_base_alter"
    dplyr::case_when(
      "SE_TXMOCANPUL" %in% names(ano_base_alter) ~ ANO >= ano_base_alter["SE_TXMOCANPUL"],
      TRUE ~ ANO >= min(ANO, na.rm = T)
    )
  ) |> 
...
df_rules_dist_se_obito60 <- data |> 
  dplyr::filter( # Filtro condicional para variáveis que estão em "ano_base_alter"
    dplyr::case_when(
      "SE_OBITO60" %in% names(ano_base_alter) ~ ANO >= ano_base_alter["SE_OBITO60"],
      TRUE ~ ANO >= min(ANO, na.rm = T)
    )
  ) |> 
...
df_rules_dist_se_tetra <- data |> 
  dplyr::filter( # Filtro condicional para variáveis que estão em "ano_base_alter"
    dplyr::case_when(
      "SE_TETRA" %in% names(ano_base_alter) ~ ANO >= ano_base_alter["SE_TETRA"],
      TRUE ~ ANO >= min(ANO, na.rm = T)
    )
  ) |> 
...
df_rules_dist_se_obitsemassis <- data |> 
  dplyr::filter( # Filtro condicional para variáveis que estão em "ano_base_alter"
    dplyr::case_when(
      "SE_OBITSEMASSIS" %in% names(ano_base_alter) ~ ANO >= ano_base_alter["SE_OBITSEMASSIS"],
      TRUE ~ ANO >= min(ANO, na.rm = T)
    )
  ) |> 
...
df_rules_dist_se_intersap <- data |> 
  dplyr::filter( # Filtro condicional para variáveis que estão em "ano_base_alter"
    dplyr::case_when(
      "SE_INTERSAP" %in% names(ano_base_alter) ~ ANO >= ano_base_alter["SE_INTERSAP"],
      TRUE ~ ANO >= min(ANO, na.rm = T)
    )
  ) |> 
...
df_rules_dist_se_intersap1a5 <- data |> 
  dplyr::filter( # Filtro condicional para variáveis que estão em "ano_base_alter"
    dplyr::case_when(
      "SE_INTERSAP1A5" %in% names(ano_base_alter) ~ ANO >= ano_base_alter["SE_INTERSAP1A5"],
      TRUE ~ ANO >= min(ANO, na.rm = T)
    )
  ) |> 
...
df_rules_dist_se_intersap60 <- data |> 
  dplyr::filter( # Filtro condicional para variáveis que estão em "ano_base_alter"
    dplyr::case_when(
      "SE_INTERSAP60" %in% names(ano_base_alter) ~ ANO >= ano_base_alter["SE_INTERSAP60"],
      TRUE ~ ANO >= min(ANO, na.rm = T)
    )
  ) |> 
...
df_rules_dist_se_fratfemur60 <- data |> 
  dplyr::filter( # Filtro condicional para variáveis que estão em "ano_base_alter"
    dplyr::case_when(
      "SE_FRATFEMUR60" %in% names(ano_base_alter) ~ ANO >= ano_base_alter["SE_FRATFEMUR60"],
      TRUE ~ ANO >= min(ANO, na.rm = T)
    )
  ) |> 
...
df_rules_dist_se_cancprost <- data |> 
  dplyr::filter( # Filtro condicional para variáveis que estão em "ano_base_alter"
    dplyr::case_when(
      "SE_CANCPROST" %in% names(ano_base_alter) ~ ANO >= ano_base_alter["SE_CANCPROST"],
      TRUE ~ ANO >= min(ANO, na.rm = T)
    )
  ) |> 
...
df_rules_dist_se_partodeslocmic <- data |> 
  dplyr::filter( # Filtro condicional para variáveis que estão em "ano_base_alter"
    dplyr::case_when(
      "SE_PARTODESLOCMIC" %in% names(ano_base_alter) ~ ANO >= ano_base_alter["SE_PARTODESLOCMIC"],
      TRUE ~ ANO >= min(ANO, na.rm = T)
    )
  ) |> 
...
#   dplyr::arrange(ano, ibge7)
# 
# df_rules_valid_se_icsabms <- df_rules_valid_se_all |> 
#   dplyr::select(
#     Ano,
#     ibge7,
#     dplyr::ends_with("SE_ICSABMS")
#   ) |> 
#   tidyr::pivot_longer(
#     cols      = !c(Ano, ibge7, dplyr::starts_with("SE_")), 
#     names_to  = "regra", 
...
#   "se_icsabms"          = df_rules_valid_se_icsabms,
...
#   dplyr::arrange(ano, ibge7)
# 
# df_rules_dist_se_icsabms <- df_rules_dist_se_icsabms |> 
#   tidyr::pivot_longer(
#     cols      = !c(Ano, ibge7, dplyr::starts_with("SE_")), 
#     names_to  = "regra", 
#     values_to = "obs_suspeita"
