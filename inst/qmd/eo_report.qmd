...
df_rules_valid_eo_all <- data |> 
  ...
  dplyr::mutate(
    ...
    TI_EO_IQEP4         = dplyr::if_else(dplyr::between(EO_IQEP4, 0, 1), F, T),
    TI_EO_IQEP8         = dplyr::if_else(dplyr::between(EO_IQEP8, 0, 1), F, T),
    TI_EO_IQE4          = dplyr::if_else(dplyr::between(EO_IQE4, 0, 1), F, T),
    TI_EO_IQE8          = dplyr::if_else(dplyr::between(EO_IQE8, 0, 1), F, T),
    TI_EO_IQE11         = dplyr::if_else(dplyr::between(EO_IQE11, 0, 1), F, T),
    TI_EO_IQE           = dplyr::if_else(dplyr::between(EO_IQE, 0, 1), F, T),
...
```{r rules_dist_EO_MED25_B}

## df com o resultado das aplicações das regras
df_rules_dist_EO_MED25_B <- data |> 
  dplyr::filter( # Filtro condicional para variáveis que estão em "ano_base_alter"
    dplyr::case_when(
      "EO_MED25_B" %in% names(ano_base_alter) ~ ANO >= ano_base_alter["EO_MED25_B"],
      TRUE ~ ANO >= min(ANO, na.rm = T)
    )
  ) |>
  dplyr::select(Ano = ANO, ibge7 = IBGE7, EO_MED25_B) |> 
  dplyr::group_by(ibge7) |> 
  dplyr::mutate(
    DF_EO_MED25_B_hampel_munic  = dplyr::if_else(!hampel_filter(EO_MED25_B), F, T), # Longitudinal (município)
    
    DF_EO_MED25_B_out_munic     = dplyr::if_else(!outlier_function(EO_MED25_B), F, T), # Longitudinal (município)
    
    DF_EO_MED25_B_min_munic     = dplyr::if_else(EO_MED25_B >= (min(dplyr::lag(EO_MED25_B), na.rm = T)), F, T), # Longitudinal (município)

    DF_EO_MED25_B_max_munic     = dplyr::if_else(EO_MED25_B <= (max(dplyr::lag(EO_MED25_B), na.rm = T)), F, T), # Longitudinal (município)

    DF_EO_MED25_B_min_k_munic   = dplyr::if_else(EO_MED25_B >= (min(dplyr::lag(EO_MED25_B), na.rm = T) / k), F, T), # Longitudinal (município)

    DF_EO_MED25_B_max_k_munic   = dplyr::if_else(EO_MED25_B <= (k * max(dplyr::lag(EO_MED25_B), na.rm = T)), F, T), # Longitudinal (município)

    DF_EO_MED25_B_med_mov_munic = dplyr::if_else(EO_MED25_B <= (k * zoo::rollmedian(EO_MED25_B, k = 3, fill = NA, align = "right")), F, T), # Longitudinal (município)

    DF_EO_MED25_B_dif_munic     = dplyr::if_else(!compare_first_dif(EO_MED25_B), F, T) # Longitudinal (município)
  )
```
...
## Transformação dos resultados
df_sumario_dist_eo <- df_rules_dist_EO_MED25_B |> 
  dplyr::select(!EO_MED25_B) |> 
  tidyr::pivot_longer(cols = !c(Ano, ibge7), names_to = "Regra", values_to = "resultado")
...
df_rules_valid <- df_rules_valid_eo_all |> 
  dplyr::select(
    Ano,
    ibge7,
    dplyr::ends_with(variables)
  ) |> 
  tidyr::pivot_longer(
    cols      = !c(Ano, ibge7, dplyr::starts_with("EO_")), 
    names_to  = "regra", 
