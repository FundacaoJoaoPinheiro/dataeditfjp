...
df_rules_dist_ea_bffam <- data |> 
  dplyr::filter( # Filtro condicional para variáveis que estão em "ano_base_alter"
    dplyr::case_when(
      "EA_BFFAM" %in% names(ano_base_alter) ~ ANO >= ano_base_alter["EA_BFFAM"],
...
df_rules_dist_ea_trfam <- data |> 
  dplyr::filter( # Filtro condicional para variáveis que estão em "ano_base_alter"
    dplyr::case_when(
      "EA_TRFAM" %in% names(ano_base_alter) ~ ANO >= ano_base_alter["EA_TRFAM"],
      TRUE ~ ANO >= min(ANO, na.rm = T)
    )
  ) |>
...
df_rules_dist_ea_trpf <- data |> 
  dplyr::filter( # Filtro condicional para variáveis que estão em "ano_base_alter"
    dplyr::case_when(
      "EA_TRPF" %in% names(ano_base_alter) ~ ANO >= ano_base_alter["EA_TRPF"],
      TRUE ~ ANO >= min(ANO, na.rm = T)
    )
  ) |>
...
df_rules_dist_ea_bpcdef <- data |> 
  dplyr::filter( # Filtro condicional para variáveis que estão em "ano_base_alter"
    dplyr::case_when(
      "EA_BPCDEF" %in% names(ano_base_alter) ~ ANO >= ano_base_alter["EA_BPCDEF"],
      TRUE ~ ANO >= min(ANO, na.rm = T)
    )
  ) |>
...
df_rules_dist_ea_bpcido <- data |> 
  dplyr::filter( # Filtro condicional para variáveis que estão em "ano_base_alter"
    dplyr::case_when(
      "EA_BPCIDO" %in% names(ano_base_alter) ~ ANO >= ano_base_alter["EA_BPCIDO"],
      TRUE ~ ANO >= min(ANO, na.rm = T)
    )
  ) |>
...
df_rules_dist_ea_bfbpc <- data |> 
  dplyr::filter( # Filtro condicional para variáveis que estão em "ano_base_alter"
    dplyr::case_when(
      "EA_BFBPC" %in% names(ano_base_alter) ~ ANO >= ano_base_alter["EA_BFBPC"],
      TRUE ~ ANO >= min(ANO, na.rm = T)
    )
  ) |>
...
df_rules_dist_ea_trbpcdef <- data |> 
  dplyr::filter( # Filtro condicional para variáveis que estão em "ano_base_alter"
    dplyr::case_when(
      "EA_TRBPCDEF" %in% names(ano_base_alter) ~ ANO >= ano_base_alter["EA_TRBPCDEF"],
      TRUE ~ ANO >= min(ANO, na.rm = T)
    )
  ) |>
...
df_rules_dist_ea_trbpcido <- data |> 
  dplyr::filter( # Filtro condicional para variáveis que estão em "ano_base_alter"
    dplyr::case_when(
      "EA_TRBPCIDO" %in% names(ano_base_alter) ~ ANO >= ano_base_alter["EA_TRBPCIDO"],
      TRUE ~ ANO >= min(ANO, na.rm = T)
    )
  ) |>
...
df_rules_dist_ea_trbpc <- data |> 
  dplyr::filter( # Filtro condicional para variáveis que estão em "ano_base_alter"
    dplyr::case_when(
      "EA_TRBPC" %in% names(ano_base_alter) ~ ANO >= ano_base_alter["EA_TRBPC"],
      TRUE ~ ANO >= min(ANO, na.rm = T)
    )
  ) |>
...
df_rules_dist_ea_trbpcpb <- data |> 
  dplyr::filter( # Filtro condicional para variáveis que estão em "ano_base_alter"
    dplyr::case_when(
      "EA_TRBPCPB" %in% names(ano_base_alter) ~ ANO >= ano_base_alter["EA_TRBPCPB"],
      TRUE ~ ANO >= min(ANO, na.rm = T)
    )
  ) |>
...
df_rules_dist_ea_trbfbpc <- data |> 
  dplyr::filter( # Filtro condicional para variáveis que estão em "ano_base_alter"
    dplyr::case_when(
      "EA_TRBFBPC" %in% names(ano_base_alter) ~ ANO >= ano_base_alter["EA_TRBFBPC"],
      TRUE ~ ANO >= min(ANO, na.rm = T)
    )
  ) |>
...
df_rules_dist_ea_trpc <- data |> 
  dplyr::filter( # Filtro condicional para variáveis que estão em "ano_base_alter"
    dplyr::case_when(
      "EA_TRPC" %in% names(ano_base_alter) ~ ANO >= ano_base_alter["EA_TRPC"],
      TRUE ~ ANO >= min(ANO, na.rm = T)
    )
  ) |>
...
df_rules_dist_ea_trpcbf <- data |> 
  dplyr::filter( # Filtro condicional para variáveis que estão em "ano_base_alter"
    dplyr::case_when(
      "EA_TRPCBF" %in% names(ano_base_alter) ~ ANO >= ano_base_alter["EA_TRPCBF"],
      TRUE ~ ANO >= min(ANO, na.rm = T)
    )
  ) |>
...
df_rules_dist_ea_trpcbpc <- data |> 
  dplyr::filter( # Filtro condicional para variáveis que estão em "ano_base_alter"
    dplyr::case_when(
      "EA_TRPCBPC" %in% names(ano_base_alter) ~ ANO >= ano_base_alter["EA_TRPCBPC"],
      TRUE ~ ANO >= min(ANO, na.rm = T)
    )
  ) |>
...
df_rules_dist_ea_vaagr <- data |> 
  dplyr::filter( # Filtro condicional para variáveis que estão em "ano_base_alter"
    dplyr::case_when(
      "EA_VAAGR" %in% names(ano_base_alter) ~ ANO >= ano_base_alter["EA_VAAGR"],
      TRUE ~ ANO >= min(ANO, na.rm = T)
    )
  ) |>
...
df_rules_dist_ea_vaind <- data |> 
  dplyr::filter( # Filtro condicional para variáveis que estão em "ano_base_alter"
    dplyr::case_when(
      "EA_VAIND" %in% names(ano_base_alter) ~ ANO >= ano_base_alter["EA_VAIND"],
      TRUE ~ ANO >= min(ANO, na.rm = T)
    )
  ) |>
...
df_rules_dist_ea_vaadm <- data |> 
  dplyr::filter( # Filtro condicional para variáveis que estão em "ano_base_alter"
    dplyr::case_when(
      "EA_VAADM" %in% names(ano_base_alter) ~ ANO >= ano_base_alter["EA_VAADM"],
      TRUE ~ ANO >= min(ANO, na.rm = T)
    )
  ) |>
...
df_rules_dist_ea_vaserv <- data |> 
  dplyr::filter( # Filtro condicional para variáveis que estão em "ano_base_alter"
    dplyr::case_when(
      "EA_VASERV" %in% names(ano_base_alter) ~ ANO >= ano_base_alter["EA_VASERV"],
      TRUE ~ ANO >= min(ANO, na.rm = T)
    )
  ) |>
...
df_rules_dist_ea_pvaagr <- data |> 
  dplyr::filter( # Filtro condicional para variáveis que estão em "ano_base_alter"
    dplyr::case_when(
      "EA_PVAAGR" %in% names(ano_base_alter) ~ ANO >= ano_base_alter["EA_PVAAGR"],
      TRUE ~ ANO >= min(ANO, na.rm = T)
    )
  ) |>
...
df_rules_dist_ea_pvaind <- data |> 
  dplyr::filter( # Filtro condicional para variáveis que estão em "ano_base_alter"
    dplyr::case_when(
      "EA_PVAIND" %in% names(ano_base_alter) ~ ANO >= ano_base_alter["EA_PVAIND"],
      TRUE ~ ANO >= min(ANO, na.rm = T)
    )
  ) |>
...
df_rules_dist_ea_pvaadm <- data |> 
  dplyr::filter( # Filtro condicional para variáveis que estão em "ano_base_alter"
    dplyr::case_when(
      "EA_PVAADM" %in% names(ano_base_alter) ~ ANO >= ano_base_alter["EA_PVAADM"],
      TRUE ~ ANO >= min(ANO, na.rm = T)
    )
  ) |>
...
df_rules_dist_ea_pvaserv <- data |> 
  dplyr::filter( # Filtro condicional para variáveis que estão em "ano_base_alter"
    dplyr::case_when(
      "EA_PVASERV" %in% names(ano_base_alter) ~ ANO >= ano_base_alter["EA_PVASERV"],
      TRUE ~ ANO >= min(ANO, na.rm = T)
    )
  ) |>
...
df_rules_dist_ea_vatot <- data |> 
  dplyr::filter( # Filtro condicional para variáveis que estão em "ano_base_alter"
    dplyr::case_when(
      "EA_VATOT" %in% names(ano_base_alter) ~ ANO >= ano_base_alter["EA_VATOT"],
      TRUE ~ ANO >= min(ANO, na.rm = T)
    )
  ) |>
...
df_rules_dist_ea_impliq <- data |> 
  dplyr::filter( # Filtro condicional para variáveis que estão em "ano_base_alter"
    dplyr::case_when(
      "EA_IMPLIQ" %in% names(ano_base_alter) ~ ANO >= ano_base_alter["EA_IMPLIQ"],
      TRUE ~ ANO >= min(ANO, na.rm = T)
    )
  ) |>
...
df_rules_dist_ea_pib <- data |> 
  dplyr::filter( # Filtro condicional para variáveis que estão em "ano_base_alter"
    dplyr::case_when(
      "EA_PIB" %in% names(ano_base_alter) ~ ANO >= ano_base_alter["EA_PIB"],
      TRUE ~ ANO >= min(ANO, na.rm = T)
    )
  ) |>
...
df_rules_dist_ea_pibpc <- data |> 
  dplyr::filter( # Filtro condicional para variáveis que estão em "ano_base_alter"
    dplyr::case_when(
      "EA_PIBPC" %in% names(ano_base_alter) ~ ANO >= ano_base_alter["EA_PIBPC"],
      TRUE ~ ANO >= min(ANO, na.rm = T)
    )
  ) |>
...
df_rules_dist_ea_vaf <- data |> 
  dplyr::filter( # Filtro condicional para variáveis que estão em "ano_base_alter"
    dplyr::case_when(
      "EA_VAF" %in% names(ano_base_alter) ~ ANO >= ano_base_alter["EA_VAF"],
      TRUE ~ ANO >= min(ANO, na.rm = T)
    )
  ) |>
...
df_rules_dist_ea_vafag <- data |> 
  dplyr::filter( # Filtro condicional para variáveis que estão em "ano_base_alter"
    dplyr::case_when(
      "EA_VAFAG" %in% names(ano_base_alter) ~ ANO >= ano_base_alter["EA_VAFAG"],
      TRUE ~ ANO >= min(ANO, na.rm = T)
    )
  ) |>
...
df_rules_dist_ea_vafmi <- data |> 
  dplyr::filter( # Filtro condicional para variáveis que estão em "ano_base_alter"
    dplyr::case_when(
      "EA_VAFMI" %in% names(ano_base_alter) ~ ANO >= ano_base_alter["EA_VAFMI"],
      TRUE ~ ANO >= min(ANO, na.rm = T)
    )
  ) |>
...
df_rules_dist_ea_vafit <- data |> 
  dplyr::filter( # Filtro condicional para variáveis que estão em "ano_base_alter"
    dplyr::case_when(
      "EA_VAFIT" %in% names(ano_base_alter) ~ ANO >= ano_base_alter["EA_VAFIT"],
      TRUE ~ ANO >= min(ANO, na.rm = T)
    )
  ) |>
...
df_rules_dist_ea_vafic <- data |> 
  dplyr::filter( # Filtro condicional para variáveis que estão em "ano_base_alter"
    dplyr::case_when(
      "EA_VAFIC" %in% names(ano_base_alter) ~ ANO >= ano_base_alter["EA_VAFIC"],
      TRUE ~ ANO >= min(ANO, na.rm = T)
    )
  ) |>
...
df_rules_dist_ea_vafup <- data |> 
  dplyr::filter( # Filtro condicional para variáveis que estão em "ano_base_alter"
    dplyr::case_when(
      "EA_VAFUP" %in% names(ano_base_alter) ~ ANO >= ano_base_alter["EA_VAFUP"],
      TRUE ~ ANO >= min(ANO, na.rm = T)
    )
  ) |>
...
df_rules_dist_ea_vafcv <- data |> 
  dplyr::filter( # Filtro condicional para variáveis que estão em "ano_base_alter"
    dplyr::case_when(
      "EA_VAFCV" %in% names(ano_base_alter) ~ ANO >= ano_base_alter["EA_VAFCV"],
      TRUE ~ ANO >= min(ANO, na.rm = T)
    )
  ) |>
...
df_rules_dist_ea_vafca <- data |> 
  dplyr::filter( # Filtro condicional para variáveis que estão em "ano_base_alter"
    dplyr::case_when(
      "EA_VAFCA" %in% names(ano_base_alter) ~ ANO >= ano_base_alter["EA_VAFCA"],
      TRUE ~ ANO >= min(ANO, na.rm = T)
    )
  ) |>
...
df_rules_dist_ea_vafse <- data |> 
  dplyr::filter( # Filtro condicional para variáveis que estão em "ano_base_alter"
    dplyr::case_when(
      "EA_VAFSE" %in% names(ano_base_alter) ~ ANO >= ano_base_alter["EA_VAFSE"],
      TRUE ~ ANO >= min(ANO, na.rm = T)
    )
  ) |>
...
df_rules_dist_ea_vafo <- data |> 
  dplyr::filter( # Filtro condicional para variáveis que estão em "ano_base_alter"
    dplyr::case_when(
      "EA_VAFO" %in% names(ano_base_alter) ~ ANO >= ano_base_alter["EA_VAFO"],
      TRUE ~ ANO >= min(ANO, na.rm = T)
    )
  ) |>
...
df_rules_dist_ea_pvafag <- data |> 
  dplyr::filter( # Filtro condicional para variáveis que estão em "ano_base_alter"
    dplyr::case_when(
      "EA_PVAFAG" %in% names(ano_base_alter) ~ ANO >= ano_base_alter["EA_PVAFAG"],
      TRUE ~ ANO >= min(ANO, na.rm = T)
    )
  ) |>
...
df_rules_dist_ea_pvafmi <- data |> 
  dplyr::filter( # Filtro condicional para variáveis que estão em "ano_base_alter"
    dplyr::case_when(
      "EA_PVAFMI" %in% names(ano_base_alter) ~ ANO >= ano_base_alter["EA_PVAFMI"],
      TRUE ~ ANO >= min(ANO, na.rm = T)
    )
  ) |>
...
df_rules_dist_ea_pvafit <- data |> 
  dplyr::filter( # Filtro condicional para variáveis que estão em "ano_base_alter"
    dplyr::case_when(
      "EA_PVAFIT" %in% names(ano_base_alter) ~ ANO >= ano_base_alter["EA_PVAFIT"],
      TRUE ~ ANO >= min(ANO, na.rm = T)
    )
  ) |>
...
df_rules_dist_ea_pvafic <- data |> 
  dplyr::filter( # Filtro condicional para variáveis que estão em "ano_base_alter"
    dplyr::case_when(
      "EA_PVAFIC" %in% names(ano_base_alter) ~ ANO >= ano_base_alter["EA_PVAFIC"],
      TRUE ~ ANO >= min(ANO, na.rm = T)
    )
  ) |>
...
df_rules_dist_ea_pvafup <- data |> 
  dplyr::filter( # Filtro condicional para variáveis que estão em "ano_base_alter"
    dplyr::case_when(
      "EA_PVAFUP" %in% names(ano_base_alter) ~ ANO >= ano_base_alter["EA_PVAFUP"],
      TRUE ~ ANO >= min(ANO, na.rm = T)
    )
  ) |>
...
df_rules_dist_ea_pvafcv <- data |> 
  dplyr::filter( # Filtro condicional para variáveis que estão em "ano_base_alter"
    dplyr::case_when(
      "EA_PVAFCV" %in% names(ano_base_alter) ~ ANO >= ano_base_alter["EA_PVAFCV"],
      TRUE ~ ANO >= min(ANO, na.rm = T)
    )
  ) |>
...
df_rules_dist_ea_pvafca <- data |> 
  dplyr::filter( # Filtro condicional para variáveis que estão em "ano_base_alter"
    dplyr::case_when(
      "EA_PVAFCA" %in% names(ano_base_alter) ~ ANO >= ano_base_alter["EA_PVAFCA"],
      TRUE ~ ANO >= min(ANO, na.rm = T)
    )
  ) |>
...
df_rules_dist_ea_pvafse <- data |> 
  dplyr::filter( # Filtro condicional para variáveis que estão em "ano_base_alter"
    dplyr::case_when(
      "EA_PVAFSE" %in% names(ano_base_alter) ~ ANO >= ano_base_alter["EA_PVAFSE"],
      TRUE ~ ANO >= min(ANO, na.rm = T)
    )
  ) |>
...
df_rules_dist_ea_pvafo <- data |> 
  dplyr::filter( # Filtro condicional para variáveis que estão em "ano_base_alter"
    dplyr::case_when(
      "EA_PVAFO" %in% names(ano_base_alter) ~ ANO >= ano_base_alter["EA_PVAFO"],
      TRUE ~ ANO >= min(ANO, na.rm = T)
    )
  ) |>
...
df_rules_dist_ea_vafpc <- data |> 
  dplyr::filter( # Filtro condicional para variáveis que estão em "ano_base_alter"
    dplyr::case_when(
      "EA_VAFPC" %in% names(ano_base_alter) ~ ANO >= ano_base_alter["EA_VAFPC"],
      TRUE ~ ANO >= min(ANO, na.rm = T)
    )
  ) |>
...
df_rules_dist_ea_vafcvpc <- data |> 
  dplyr::filter( # Filtro condicional para variáveis que estão em "ano_base_alter"
    dplyr::case_when(
      "EA_VAFCVPC" %in% names(ano_base_alter) ~ ANO >= ano_base_alter["EA_VAFCVPC"],
      TRUE ~ ANO >= min(ANO, na.rm = T)
    )
  ) |>
...
df_rules_dist_ea_emprsf <- data |> 
  dplyr::filter( # Filtro condicional para variáveis que estão em "ano_base_alter"
    dplyr::case_when(
      "EA_EMPRSF" %in% names(ano_base_alter) ~ ANO >= ano_base_alter["EA_EMPRSF"],
      TRUE ~ ANO >= min(ANO, na.rm = T)
    )
  ) |>
...
df_rules_dist_ea_emprsftx <- data |> 
  dplyr::filter( # Filtro condicional para variáveis que estão em "ano_base_alter"
    dplyr::case_when(
      "EA_EMPRSFTX" %in% names(ano_base_alter) ~ ANO >= ano_base_alter["EA_EMPRSFTX"],
      TRUE ~ ANO >= min(ANO, na.rm = T)
    )
  ) |>
...
df_rules_dist_ea_rensf <- data |> 
  dplyr::filter( # Filtro condicional para variáveis que estão em "ano_base_alter"
    dplyr::case_when(
      "EA_RENSF" %in% names(ano_base_alter) ~ ANO >= ano_base_alter["EA_RENSF"],
      TRUE ~ ANO >= min(ANO, na.rm = T)
    )
  ) |>
...
df_rules_dist_ea_renpcsf <- data |> 
   dplyr::filter( # Filtro condicional para variáveis que estão em "ano_base_alter"
    dplyr::case_when(
      "EA_RENPCSF" %in% names(ano_base_alter) ~ ANO >= ano_base_alter["EA_RENPCSF"],
      TRUE ~ ANO >= min(ANO, na.rm = T)
    )
  ) |>
...
df_rules_dist_ea_emprsfag <- data |> 
  dplyr::filter( # Filtro condicional para variáveis que estão em "ano_base_alter"
    dplyr::case_when(
      "EA_EMPRSFAG" %in% names(ano_base_alter) ~ ANO >= ano_base_alter["EA_EMPRSFAG"],
      TRUE ~ ANO >= min(ANO, na.rm = T)
    )
  ) |>
...
df_rules_dist_ea_emprsfmi <- data |> 
  dplyr::filter( # Filtro condicional para variáveis que estão em "ano_base_alter"
    dplyr::case_when(
      "EA_EMPRSFMI" %in% names(ano_base_alter) ~ ANO >= ano_base_alter["EA_EMPRSFMI"],
      TRUE ~ ANO >= min(ANO, na.rm = T)
    )
  ) |>
...
df_rules_dist_ea_emprsfit <- data |> 
  dplyr::filter( # Filtro condicional para variáveis que estão em "ano_base_alter"
    dplyr::case_when(
      "EA_EMPRSFIT" %in% names(ano_base_alter) ~ ANO >= ano_base_alter["EA_EMPRSFIT"],
      TRUE ~ ANO >= min(ANO, na.rm = T)
    )
  ) |>
...
df_rules_dist_ea_emprsfup <- data |> 
  dplyr::filter( # Filtro condicional para variáveis que estão em "ano_base_alter"
    dplyr::case_when(
      "EA_EMPRSFUP" %in% names(ano_base_alter) ~ ANO >= ano_base_alter["EA_EMPRSFUP"],
      TRUE ~ ANO >= min(ANO, na.rm = T)
    )
  ) |>
...
df_rules_dist_ea_emprsfic <- data |> 
  dplyr::filter( # Filtro condicional para variáveis que estão em "ano_base_alter"
    dplyr::case_when(
      "EA_EMPRSFIC" %in% names(ano_base_alter) ~ ANO >= ano_base_alter["EA_EMPRSFIC"],
      TRUE ~ ANO >= min(ANO, na.rm = T)
    )
  ) |>
...
df_rules_dist_ea_emprsfco <- data |> 
  dplyr::filter( # Filtro condicional para variáveis que estão em "ano_base_alter"
    dplyr::case_when(
      "EA_EMPRSFCO" %in% names(ano_base_alter) ~ ANO >= ano_base_alter["EA_EMPRSFCO"],
      TRUE ~ ANO >= min(ANO, na.rm = T)
    )
  ) |>
...
df_rules_dist_ea_emprsfse <- data |> 
  dplyr::filter( # Filtro condicional para variáveis que estão em "ano_base_alter"
    dplyr::case_when(
      "EA_EMPRSFSE" %in% names(ano_base_alter) ~ ANO >= ano_base_alter["EA_EMPRSFSE"],
      TRUE ~ ANO >= min(ANO, na.rm = T)
    )
  ) |>
