...
ano_base_alter <- c(
   ...
   "GP_CONSCA"            = 2019,
   "GP_CONSMULHER"        = 2005,
   # "GP_TIPOSCONSELHOS"    = 2017,
   "GP_CCONVMAMB"         = 2011,
   "GP_ESTRUTPLANEJORCA"  = 2017,
   "GP_TREINASERVPLANEJA" = 2017,
   "GP_TREINASERVDEMAIS"  = 2017,
   "GP_TREINASERVTI"      = 2017,
   "GP_PMEDUCA"           = 2017,
   "GP_PMSAUDE"           = 2017,
   "GP_PMASSOC"           = 2017,
   "GP_LEIACESSOINFOR"    = 2017,
   "GP_DIVULGORCA"        = 2017,
   "GP_DIVULGESTAOFISCAL" = 2017,
   "GP_SITEATUAL"         = 2017,
   "GP_CONSULMEDUBS"      = 2017,
   "GP_SISTCONTPONTOMED"  = 2017,
   "GP_SISTINFPLANEJ"     = 2017,
   "GP_ARRECADISS"        = 2017,
   "GP_CONTROLAIPTU"      = 2017,
   "GP_CONTRATOSINTERNET" = 2017,
   "GP_EDITAISLICITA"     = 2017,
   "GP_ATASDIVULG"        = 2017,
   "GP_TECCOMPRASPUB"     = 2017,
   "GP_ESTOQUESAUDE"      = 2017,
   "GP_PLPROTDEFCIVIL"    = 2017,
   "GP_INDTRANSPGES"      = 2017,
   "GP_INDIGITGES"        = 2020,
   "GP_FUNCULT"           = 2018,
   "GP_FUNDSAU"           = 2018,
   # "GP_FUNDMUNIC"         = 2017,
   "GP_PLANODIRETOR"      = 2017,
   "GP_PMSANEA"           = 2022,
   "GP_PMRESIDUOS"        = 2022,
   "GP_CONSDIRHUM"        = 2023,
   "GP_CONSSEGPUB"        = 2023
...
desc_regra_valid_var <- data.frame(
  ...
  dplyr::mutate(
    `Descrição` = dplyr::case_match(
      ...
      "VI_GP_PMSANEA"           ~ desc_regra_valid[2],
      "VI_GP_PMRESIDUOS"        ~ desc_regra_valid[2],
      "VI_GP_CONSDIRHUM"        ~ desc_regra_valid[2],
      "VI_GP_CONSSEGPUB"        ~ desc_regra_valid[2]
    )
  )
```
...
rules_type <- validate::validator(
  ...
  TI_GP_PMSANEA           = is.character(GP_PMSANEA),       
  TI_GP_PMRESIDUOS        = is.character(GP_PMRESIDUOS),
  TI_GP_CONSDIRHUM        = is.character(GP_CONSDIRHUM),
  TI_GP_CONSSEGPUB        = is.character(GP_CONSSEGPUB)
)
```
...
df_rules_valid_gp_all <- data |> 
  ...
  dplyr::mutate(
    ...
    VI_NA_GP_PMSANEA           = dplyr::if_else(!is.na(GP_PMSANEA), F, T),
    VI_NA_GP_PMRESIDUOS        = dplyr::if_else(!is.na(GP_PMRESIDUOS), F, T),
    VI_NA_GP_CONSDIRHUM        = dplyr::if_else(!is.na(GP_CONSDIRHUM), F, T),
    VI_NA_GP_CONSSEGPUB        = dplyr::if_else(!is.na(GP_CONSSEGPUB), F, T),
    # Others
    ...
    VI_GP_PMSANEA           = dplyr::if_else(GP_PMSANEA %in% cat_dic, F, T),
    VI_GP_PMRESIDUOS        = dplyr::if_else(GP_PMRESIDUOS %in% cat_dic, F, T),
    VI_GP_CONSDIRHUM        = dplyr::if_else(GP_CONSDIRHUM %in% cat_dic, F, T),
    VI_GP_CONSSEGPUB        = dplyr::if_else(GP_CONSSEGPUB %in% cat_dic, F, T)
  )
```
