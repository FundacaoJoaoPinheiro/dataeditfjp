...
```{r map_consist_desc}
 ...
 desc_regra_consist <- c(
   `1` = "A soma de CA_PCL e CA_APRESPC é igual a CA_GPRESPC"
 )
...
desc_regra_consist_var <- data.frame(
   ...
   dplyr::mutate(
     `Descrição` = dplyr::case_match(
       Regra,
       "CA_GPRESPC_total" ~ desc_regra_consist[1]
     )
   )
```
...
df_rules_consist_ca_all <- data |> 
  ...
  dplyr::group_by(Ano) |> 
  dplyr::mutate(
    CA_GPRESPC_soma  = CA_PCL + CA_APRESPC,
    #CA_GPRESPC_soma  = dplyr::if_else(dplyr::between(CA_GPRESPC_soma, 0.999*CA_GPRESPC, 1.001*CA_GPRESPC), F, T),
    CA_GPRESPC_total  = dplyr::if_else(CA_GPRESPC == CA_GPRESPC_soma, F, T)
  ) |> dplyr::select(Ano,ibge7, CA_GPRESPC_total, CA_GPRESPC) |> dplyr::ungroup()
```
...
sapply(function(i){
lis[[i]] <- df_rules_consist_ca_all |>
  dplyr::select(
    Ano,
    ibge7,
    dplyr::contains(i)
  ) |> dplyr::filter(CA_GPRESPC_total == T) |> dplyr::select(!CA_GPRESPC_total) |> 
  tidyr::pivot_longer(
    cols      = !c(Ano, ibge7),
    names_to  = "regra",
    values_to = "obs_suspeita"
  ...
  dplyr::rename_with(tolower) |>
  dplyr::left_join(cidades, by = join_by(ibge7==id)) |>
  dplyr::select(ano, ibge7, obs_suspeita, regra, cidade) |>
  dplyr::arrange(ano, ibge7)
return(lis[[i]])
}, USE.NAMES=TRUE, simplify = FALSE)
