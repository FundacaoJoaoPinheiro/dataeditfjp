...
data_aux <- sapply(names(data_aux),
       function(i){
         if(names(data_aux[i]) %in% var_numeric){
          data_aux <- data_aux %>% dplyr::select(IBGE7,ANO,!!newnames[i] := i)
         }
         return(data_aux)
...
var_char <- data |> 
  dplyr::select(
    !c("CHAVE", "IBGE6", "IBGE7", "ANO") & !!categoricas
  ) |> 
  colnames() |> 
  unlist()
...
-   **DF\_[...]\_meddiana_k_movel**: o valor atual é menor ou igual a $mediana ~móvel \times k$;
...
strsplit(x = categorias$VI_CATEGORY, split = "[ "," ]" )
...
desc_regra_valid_var <- data.frame(
  Regra = paste0("variable",seq(1,length(var_numeric)))
  ) |> 
  dplyr::mutate(
