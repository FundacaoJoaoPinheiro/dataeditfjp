...
numericas <- file_rules %>% dplyr::filter(TYPE == "numeric") %>% dplyr::select(VARIABLE) %>% dplyr::pull()

categoricas <- file_rules %>% dplyr::filter(TYPE != "numeric") %>% dplyr::select(VARIABLE) %>% dplyr::pull()

var_numeric <- data |> 
  dplyr::select(
    !c("CHAVE", "IBGE6", "IBGE7", "ANO") & !!numericas
  ) |> 
  colnames() |> 
  unlist()


data_aux1 <- data %>% dplyr::select(-all_of(categoricas))
data_aux2 <- data %>% dplyr::select(-all_of(numericas))

# newnames <- seq(1,length(var_numeric))
# newnames <- paste0("variable",newnames)
# names(newnames) <- var_numeric


# data_aux <- sapply(names(newnames),
#        function(i){
#          if(names(data_aux[i]) %in% var_numeric){
#           data_aux <- data_aux %>% dplyr::select(IBGE7,ANO, eval_tidy(quote(i)))
#          }
#          return(data_aux)
...
categorias <- file_rules %>% dplyr::filter(TYPE == "categorical") %>% dplyr::select(VI_CATEGORY)
categorias <- strsplit(x = categorias$VI_CATEGORY, split = ";" ) |> unlist() 
dicotomica <- c("Sim", "Não")
...
if(is.null(begin_years)){
  ...
  ano_base_alter <- params$begin_years
}
...
#exclusions_years <- deleted_years
exclusions_years = c(
   "NA" = NA_integer_
)
...
desc_regra_valid <- c(
  # Casos mais comuns
  `1` = "Value is greater than or equal to 0",
  `2` = "Value belongs to the expected category",
  `3` = "Value belongs to the expected range (0 a 1)",
  `4` = "Value belongs to the expected range (0 a 10)",
  `5` = "Value belongs to the expected range (0 a 100)",
  `6` = "Value belongs to the expected range (0 a 1000)",
  `7` = "Value belongs to the expected range (0 a 10000)",
  `8` = "Value belongs to the expected range (1 a 5)",
  # Casos especiais
  `9`= "Value belongs to the expected range",
  `10` = "Value is greater than or equal to 1", # SP_PM e SP_PMPC
  `11`= "Value is greater than or equal to 8" # SP_PM1
  
)

file_rules_aux <- file_rules |> 
  dplyr::mutate(dplyr::across(dplyr::where(is.numeric), ~as.character(.x)) )|> 
  dplyr::select(VARIABLE, grep(pattern = "VI", x = names(file_rules))) |>
  tidyr::pivot_longer(cols = !c(VARIABLE), names_to = "rules", values_to = "result")


# df com o nome da regra e a respectiva descrição
desc_regra_valid_var <- file_rules_aux |> 
  dplyr::mutate(
    Description = case_when(
      rules == "VI_MORETHANZERO" & result == 1 ~ desc_regra_valid[1],
      rules == "VI_0TO100" & result == 1       ~ desc_regra_valid[5]
...
rules_type <- validate::validator(
   variable1       = is.numeric(variable1),
   variable2       = is.numeric(variable2),
   variable3       = is.numeric(variable3),
   variable4       = is.numeric(variable4),
   variable5       = is.numeric(variable5),
   variable6       = is.numeric(variable6),
   variable7       = is.numeric(variable7),
   variable8       = is.numeric(variable8),
   variable9       = is.numeric(variable9)#,
  # variable10      = is.numeric(variable10), ### categorica
  # variable11      = is.numeric(variable11), ### categorica. Eh quantitativa
  # variable12      = is.numeric(variable12) ### categorica
)
```
...
check_type <- validate::confront(data_aux, rules_type)
...
df_rules_valid_as_all <- data |> 
  ...
  dplyr::mutate(
    ...
    VI_AS_IDCRASME                = dplyr::if_else(dplyr::between(AS_IDCRASME, 0, 1), F, T),
...
df_rules_consist_as_all <- data |> 
  ...
  ) |> dplyr::select(Ano,ibge7, CF_AS_BPC_total_year) |> dplyr::ungroup()
```
...
```{r rules_dist_as_rhtotalb, eval=FALSE}
...
```{r data_wrangling_dist_as_rhtotalb, eval=FALSE}
...
```{r table_dist_as_rhtotalb, eval=FALSE}
#| column: screen-inset-right
...
```{r chart_dist_as_rhtotalb, eval=FALSE}
#| column: screen-inset-right
#| layout-ncol: 2
#| out-width: 100%
...
```{r rules_dist_as_poppobextrpob, eval=FALSE}
...
```{r data_wrangling_dist_as_poppobextrpob, eval=FALSE}
...
```{r table_dist_as_poppobextrpob, eval=FALSE}
#| column: screen-inset-right
...
```{r chart_dist_as_poppobextrpob, eval=FALSE}
#| column: screen-inset-right
#| layout-ncol: 2
#| out-width: 100%
...
df_rules_dist_as_igdmconde <- data |> 
  remove_year_variable(variable = "AS_IGDMCONDE", exclusions_years = exclusions_years) |>
  dplyr::filter( # Filtro condicional para variáveis que estão em "ano_base_alter"
    dplyr::case_when(
...
df_rules_dist_as_igdmconds <- data |> 
  remove_year_variable(variable = "AS_IGDMCONDS", exclusions_years = exclusions_years) |>
  dplyr::filter( # Filtro condicional para variáveis que estão em "ano_base_alter"
    dplyr::case_when(
